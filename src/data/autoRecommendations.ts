...
export type RuleCondition = FieldCondition | { all: RuleCondition[] } | { any: RuleCondition[] };

export interface AutoRecommendationRule {
  if: RuleCondition;
  then: string[];
  why: string;
  evidence: string[];
...
export const AUTO_RECOMMENDATION_RULES: AutoRecommendationRule[] = [
  {
  ...
  },
  {
    ...
    then: ['retinography', 'delay_final_rx_if_recent_hyperglycemia'],
